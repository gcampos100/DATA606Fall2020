...
\	
**ANSWER:** $Rows:$ 1,458 $Columns:$ 123
	
```{r dimensions, echo=TRUE, results='hide'}
glimpse(hfi)
```
...
**ANSWER:** I would use a scatter plot, which shows their is some linearity to the points. I do not feel comfortable using a linear model for predicting the personal freedom score, the data is to scattered with outlyers, and I would be unsure of what type of plot to use.
    
```{r Question 2, echo = FALSE, results='hide',warning=FALSE, fig.show='show', fig.height= 3}

ggplot(hfi, aes(x=pf_expression_control, y=pf_score)) +
  geom_smooth(method = "lm", color = "green")+
  geom_point(aes(col=pf_score))+
  labs(subtitle="Personal freedom score and personal freedem expression control (Linear Model 'lm')", 
       y="PF Score", 
...
ggplot(hfi, aes(x=pf_expression_control, y=pf_score)) +
  ...
  geom_point(aes(col=pf_score))+
  labs(subtitle="Personal freedom score and personal freedem expression control\n(Local Estimated Scatterplot Smoothing LEOSS)", 
       y="PF Score", 
       x="PF Expression Control", 
       title="Scatterplot", 
       caption = "Source: Human Freedom Index")
...
\
**ANSWER:** From the bottom left to the top right, it's clear to see a positive correlation between the two variables visually and the correlation coefficient of 0.796 supports the relationship is a strong one.There are however a great deal of plots, including outlyers to consider regarding the data. 
...
```{r plotss-expression-score-corrected, eval=TRUE,echo=FALSE, fig.show='show'}
#Let's make a subset of the data needed
hfi_subset<-hfi[,c("pf_expression_control", "pf_score")]
#lets also remove the NA values in the row
hfi_no.na<-hfi_subset[complete.cases(hfi_subset),]
plot_ss(x = hfi_no.na$pf_expression_control, y = hfi_no.na$pf_score)+
	title("Plot_ss with NA removed")
```
...
```{r plotss-expression-score-squares, eval=FALSE, message=TRUE}
...
\
**Stefano Biguzzi  9:50 AM**\newline
*For **lab 8 question** 4 it asks to compare your answer to your neighbors. The smallest Sum of Least squares I got was **989.955** * 
    
```{r, echo=FALSE, results='show', fig.show='show', fig.height=3}
plot_ss(x = hfi_no.na$pf_expression_control, y = hfi_no.na$pf_score,showSquares = TRUE)
```
...
column displays the linear model's y-intercept and the coefficient of `pf_score`.
...
\
**ANSWER:** The slop indicates a positive relationship with and small incremental increase or\newline
mild steepness.
...
```{r Q5, results='hide'}
m5 <- lm(hf_score ~ pf_expression_control, data = hfi)
summary(m5)
```
...
```{r reg-with-line,warning=FALSE, message=FALSE, fig.show='show',fig.height=2}
...
\
**ANSWER:** You could substitute $\hat y$ with $6.7$ which is a relatively close\newline
result since the pf_scores at pf_expression_control 7.5~ is between 7-8.5 
\[
  \hat y = 5.153687 + 0.349862 \times 6.7
\]
\[ \hat y = 7.497762 \]



```{r calc-Q6, include = FALSE}
5.153687+0.349862*6.7
```


```{r plotss-expression-score-q6, eval=TRUE,echo=FALSE, fig.show='hide'}
#Let's make a subset of the data needed
hfi_subset_q6<-hfi[,c("pf_expression_control", "hf_score")]
#lets also remove the NA values in the row
hfi_no.na.q6<-hfi_subset_q6[complete.cases(hfi_subset_q6),]
plot_ss(x = hfi_no.na.q6$pf_expression_control, y = hfi_no.na.q6$hf_score,leastSquares = TRUE)+
	title("Q6 Plot_ss with NA removed")
...
```{r residuals, fig.show='show', fig.height=3}
...
**ANSWER:** No real pattern outside of appearing to be nearly normally distributed or possibly linear

</br>

**Nearly normal residuals**: To check this condition, we can look at a histogram

```{r hist-res, echo=FALSE, fig.show='show'}
...
```{r qq-res, echo=FALSE, fig.show='show'}
...
**ANSWER:** Yes it appears nearly normal
...
**ANSWER:** Yes constant variability condition appears to be met.
...
\    
**ANSWER:** New Variable is pf_movements. Their appears to be a somewhat linear and positive relationship.
    
```{r Q10_plot,echo=FALSE,warning=FALSE, fig.show='show',fig.height=3}
ggplot(hfi, aes(x=pf_movement, y=pf_score)) +
  geom_smooth(method = "lm", color = "green")+
  geom_point(aes(col=pf_score))+
  labs(subtitle="Personal freedom score and personal freedem movement", 
       y="PF Score", 
       x="PF Movement", 
       title="Scatterplot", 
       caption = "Source: Human Freedom Index")
```
    ...
    ```{r residuals_10,include=FALSE}
m10 <- lm(pf_score ~ pf_movement, data = hfi)
summary(m10)
```

```{r scatter_q10, echo=FALSE, fig.show='show',fig.height=3}
...
**ANSWER:** $R^2$-value for my 'movement' model is 61.02% compared to my original percentage of 63.42% which is a slight decrease making the original model better since it will be accounting for a larger variation.

```{r Q11, echo=TRUE, results=FALSE}
hfi%>%
summarise(cor(pf_movement, pf_movement, use = "complete.obs"))
summary(m10)
summary(m1)
```
...
**ANSWER:** I was not surprised the personal freedom movement was as closely tied to personal freedom control as it was, I feel those to variables are like seeing different sides of the same coin.
