---
title: "DATA 606 Fall 2020 - Final Exam"
...
3.  **(d)** $Both\ studies\ (a)\ and\ (b)\ can\ be\ conducted\ in\ order\ to\ establish\ that\ the$ 
			$treatment\ does\ indeed\ cause\ improvement\ with\ regards\ to\ fever\ in\ Ebola\ patients.$
...
* B $Sample Distribution$ is nearly normal.
...
data1.y.sd <- as.numeric(format(round(sd(data1$x),4), nsmall = 4))
data2.x.sd <- as.numeric(format(round(sd(data2$x),4), nsmall = 4))
data2.y.sd <- as.numeric(format(round(sd(data2$x),4), nsmall = 4))
data3.x.sd <- as.numeric(format(round(sd(data3$x),4), nsmall = 4))
data3.y.sd <- as.numeric(format(round(sd(data3$x),4), nsmall = 4))
```
...
```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
hist(data1$x,main = "Histogram of Data1 x values", col = 'light green',xlab = "X")
plot_ss(x = data1$x, y = data1$y, showSquares = TRUE)+title("Scatter plot of Data1")
```

```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
qqnorm(data1$x,main = "Data1 Q-Q Plot")
qqline(data1$x)
```
...
* **Constant Variability-** Condition not met met based on residual plot
* **Independence Observation-** Condition met

```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
hist(data2$x,main = "Histogram of Data2 x values", col = 'light green',xlab = "X")
plot_ss(x = data2$x, y = data2$y, showSquares = TRUE)+title("Scatter plot of Data2")
```

```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
qqnorm(data2$x,main = "Data2 Q-Q Plot")
qqline(data2$x)
```
...
```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
hist(data3$x,main = "Histogram of Data3 x values", col = 'light green',xlab = "X")
plot_ss(x = data3$x, y = data3$y, showSquares = TRUE)+title("Scatter plot of Data3")
```

```{r,echo=FALSE,message="hide",results="hide",include=TRUE,fig.show="show", out.width="40%"}
qqnorm(data3$x,main = "Data3 Q-Q Plot")
qqline(data3$x)
```
...
* Multiple visualizations helps avoid making assumptions, e.g. some conditions were met when deciding if linear
regression model is appropriate, but not all. Deciding based on only one would have led to the wrong assumption
