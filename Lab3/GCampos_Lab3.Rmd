...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
   ...
   **i)Streaks are based on consecutive hits. \newline ii)Its length is tied directly to the summation of hits. \newline iii)The event needed to initialize a streak is a hit (unless we were calculating a streak of missed shots, which we are not).\newline \newline Therefore a streak length of 1 is a hit followed by a miss and streak length of 0 is a miss, since again a streak is NOT initialized by a miss.** 
   
\[ \textrm{H M | M | H H M | M | M | M} \]
\[ \textrm{1+0 | 0 | 1+1+0 | 0 | 0 | 0} \]
\[ \textrm{1 | 0 | 2 | 0 | 0 | 0} \]
...
**The graph shows that a streak of 0 is most common, followed by a streak of 1. The longest streak was 4 hits consecutively**
\newline
...
# Commented out to prevent pop up
#?sample
```
...
```{r sim-basket}
shot_outcomes <- c("H", "M")
sim_basket_demo <- sample(shot_outcomes, size = 1, replace = TRUE)
```
...
```{r sim-basket-45}
#Shot outcomes vector already created above so all that is needed is the manipulation of probability
set.seed(3606)
...
**The typical streak is 0 and the longest streak is 4**

```{r summariase-sim_streak}
sim_streak%>%
	summarise(mean_strk = mean(length),
	median_strk = median(length),
	iqr_strk = IQR(length),
	sd_strk = sd(length),
	n_strk = n())
```
...
**If the simulation were ran again, with the same percentages the likelihood of the similar results would be high.**

```{r calc-sim_streak_45_2}
#Shot outcomes vector already created above so all that is needed is the manipulation of probability
set.seed(3608)
sim_basket_2 <- sample(shot_outcomes, size = 133, replace = TRUE, 
                          prob = c(0.45, 0.55))
#calc streak for new vector
sim_streak_2 <- calc_streak(sim_basket_2)
#plot new streak
ggplot(data = sim_streak_2, aes(x = length)) +
  geom_bar(alpha = 0.7)
#summarize new streak
sim_streak_2%>%
	summarise(mean_strk = mean(length),
	median_strk_2 = median(length),
	iqr_strk_2 = IQR(length),
	sd_strk_2 = sd(length),
	n_strk_2 = n())
```
...
**The distribution is similar, which supports the conclusion that the "hot-hands" theory is not valid**
