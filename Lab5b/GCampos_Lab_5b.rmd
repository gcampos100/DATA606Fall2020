...
\
    ...
    local community.
\
**The sample samp shows that *66.7%* of the population thinks climate change affests their local community**
    
```{r Exercise_1, results = TRUE, echo= FALSE}
samp %>%
  count(climate_change_affects) %>%
  mutate(p = n /sum(n))
```
...
\
**A sample size of 60 is still relatively large and results should be somewhat *similar* but not necessarily exact**
...
- In `generate` we provide the number of re-samples we want from the population in the `reps` argument (this should be a reasonably large number) as well as the type of re-sampling we want to do, which is `"bootstrap"` in the case of constructing a confidence interval.
- Then, we `calculate` the sample statistic of interest for each of these re-samples, which is `proportion`.
...
\
**It's the range of values where we have a 95% of confidence that the value contains the true mean of the population, or where the sample population mean is accurate compared to the actual population**\newline
...
\
**Yes. The TRUE population proportion of 62% is captured in this confidence interval. I am not working this lab in a classroom**

```{r Q4, results = FALSE,echo=FALSE}
samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
...
**By definition, I would expect that 95% of the intervals to capture the true population mean.**\newline
...
```{r shiny, echo=FALSE, eval=FALSE, results = TRUE, include=FALSE}
...
\
**Based on the findings below, 3 out the 50 did not include the true population proportion, meaning that is not exactly equal to the confidence level, being short by 1%** \newline 
**(*94%<95%*)**\newline
\
...
**Decreasing the confidence interval, which is what I did, decreases the error boundaries, and can physically be seen on the boundaries in a normal curve. This in turn makes the confidence interval *NARROWER* than the interval for 95%**
...
**A confidence level of 99% gave me the limits .517-.817 indicating that with 99% certainty the true mean lies between the intervals .517-.817**
...
**1 out of the 50 samples was outside the interval which is consistent with the 99% confidence, providing us a 98% accuracy**
...
**A confidence level of 25% gave me the limits .65-683, I expected a very narrow interval considering the low confidence level. The proportion capture is 19/100 intervals or 19% which is relatively close to our confidence level.**
...
**The widths of the intervals decrease.**
...
**The width of the interval change increases with the number of boot straps, which makes sense since increasing bootstraps increases SE b/c the mean will have more variability**
