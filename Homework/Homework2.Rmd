...
library(ggplot2)
score_val <- data.frame(group = "Introductory Statistical Scores", scores = scores)
ggplot(score_val, aes (y=scores)) + ylim(60,100) + geom_boxplot() + coord_flip()
```
--------------------------------------------------------------------------------
...
\
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=2}
library(openintro)
set.seed(7365)
sym = rnorm(1000, mean = 60, sd = 3)
uni = runif(1000, min = 0, max = 100)
...
**25% = Q1 = $350,000 | 50% = Q2 = $450,000 | 75% = Q3 = $1,000,000 ** \newline
\newline
...
alive at the end of the study, and *dead* on ___ [75] ___ cards 
representing patients who were not. Then, we shuffle these cards and split them 
into two groups: one group of size ___ [69] ___ representing treatment, and 
another group of size ___ [34] ____ representing control. We calculate the 
...
times to build a distribution centered at __ [0] __. Lastly, we calculate 
the fraction of simulations where the simulated differences in proportions are 
__ [0.230179] __. If this fraction is low, we conclude that it is unlikely to 
