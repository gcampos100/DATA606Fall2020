...
arbuthnot$girls
```
**to extract the counts of girls resulting in:**\newline
[1] 4683 4457 4102 4590 4839 4820 4928 4605 4457 4952 4784 5332 5200 4910 4617 3997 3919 3395 3536 3181 2746 2722 2840\newline
[24] 2908 2959 3179 3349 3382 3289 3013 2781 3247 4107 4803 4881 5681 4858 4319 5322 5560 5829 5719 6061 6120 5822 5738\newline
[47] 5717 5847 6203 6033 6041 6299 6533 6744 7158 7127 7246 7119 7214 7101 7167 7302 7392 7316 7483 6647 6713 7229 7767\newline
...
# commented out to avoid loading page during knit
# ?ggplot
```
...
1. Is there an apparent trend in the number of girls baptized over the years? 
   ...
   interpretation.)\newline
**There is a increase in girl baptisms until 1640, after which girl baptisms decline until approximately 1670.\newline \newline In general girl baptisms continue to increase after 1670.**
   
```{r plot-girls-vs-year-line-and-point}
ggplot(data = arbuthnot, aes(x = year, y = girls),environment = parent.frame()) + 
...
**I see that the data is volatile, with a constant fluctuation of boy baptisms in short yearly intervals, and in general a decline over a period of time**

```{r plot-prop-boys-vs-year}
	ggplot(data=arbuthnot, mapping = aes(x = year, y = boy_ratio))+geom_line()+geom_point()
```
...
*NOTE: considering glimpse function does not call all the years I chose to use the subsetting or $ operator for the first question*
```{r view-present-year-count}
	present$year
```
<p>
**results:\newline present$year**\newline
[1] 1940 1941 1942 1943 1944 1945 1946 1947 1948 1949 1950 1951 1952 1953 1954 1955 1956 1957 1958 1959 1960 1961 1962\newline
[24] 1963 1964 1965 1966 1967 1968 1969 1970 1971 1972 1973 1974 1975 1976 1977 1978 1979 1980 1981 1982 1983 1984 1985\newline
[47] 1986 1987 1988 1989 1990 1991 1992 1993 1994 1995 1996 1997 1998 1999 2000 2001 2002\newline
</p>

```{r glimpse-present-data-result}
	glimpse(present)
```
<p>
**result:\newline glimpse(present)**\newline
Rows: 63\newline
Columns: 3\newline
$ year  <dbl> 1940, 1941, 1942, 1943, 1944, 1945, 1946, 1947, 1948, 1949, 1950, 1951, 1952, 1953, 1954, 1955, 1956, ...\newline
$ boys  <dbl> 1211684, 1289734, 1444365, 1508959, 1435301, 1404587, 1691220, 1899876, 1813852, 1826352, 1823555, 192...\newline
$ girls <dbl> 1148715, 1223693, 1364631, 1427901, 1359499, 1330869, 1597452, 1800064, 1721216, 1733177, 1730594, 182...\newline
</p>

2.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

```{r calc-total-bapt-present-save}
present <- present %>%
  mutate(total = boys + girls)
```

```{r summary-max-min-present-arbuthot-data}
# Abuthnot Min and Max
	arbuthnot %>%
	summarize(min = min(total), max = max(total))
# Present Min and Max
	present %>%
	summarize(min = min(total), max = max(total))
# Dr. Arbuthnot's maximum compared to present minimum
2360399/16145
'equals to [1] 146.2'
```
	
<p>
The counts are not similar as noted below:\newline \newline \ **Abuthnot Min and Max** \newline
**min   max**\newline
**5612  16145**\newline \newline
**Present Min and Max**\newline
**min     max**\newline
...
Where maximum number of births during Dr. Arbuthnot's study was **146 times less than the minimum birthrate magnitude**, for the 'present" dataset.
</p>
...
```{r calc-prop-boys-var}
present <- present %>%
	mutate(boys_ratio = boys / total)
```
    
    
```{r plot-prop-boys-vs-present-year}
ggplot(data=present, mapping = aes(x = year, y = boys_ratio))+geom_line()+geom_point()
```

```{r calc-boys-more-than-girls}
present<-present%>%
	mutate(more_boys = boys > girls)
```
```{r view-more-boys-than-girls-results}
present$more_boys
```
...
[47] TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE TRUE
</p>
**Answer:**\newline
The plot show a **negative trend** for male birth rates over time.\newline
The trend that boys are being born in a greater proportion than girls still holds up in the U.S.
